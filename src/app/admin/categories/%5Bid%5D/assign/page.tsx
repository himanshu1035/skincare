      ...
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-500 text-xs mt-1">Select products for this category or click edit to change details.</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {allProducts?.map((product) => {
          ...
          return (
            <div 
              key={product.skin_id}
              className={`p-4 rounded-xl border-2 transition-all cursor-pointer group flex flex-col gap-4 bg-white relative ${
                isAssigned ? 'border-black' : 'border-gray-100 hover:border-gray-300'
              }`}
            >
              {/* Edit Link (Absolute positioned) */}
              <Link 
                href={`/admin/products/${product.skin_id}`}
                className="absolute top-2 left-2 z-20 w-8 h-8 bg-white border border-gray-200 rounded-lg flex items-center justify-center text-gray-400 hover:text-black hover:shadow-md transition-all opacity-0 group-hover:opacity-100"
                title="Edit Product"
              >
                <Edit3 size={14} />
              </Link>
