...
function OrdersContent() {
  ...
  const fetchOrders = async () => {
    ...
    let query = supabase
      ...
      .select('*, skin_orders(skin_id, skin_first_name, skin_last_name, skin_customer_email, skin_items, skin_coupon_code, skin_created_at)')
      .eq('skin_marketer_id', session.user.id);

    if (couponFilter) {
      // Filter by the coupon code in the joined skin_orders table
      // Note: In Supabase, filtering on joined tables can be tricky, 
      // but here we can filter by the coupon code if we know it.
    }
      ...
      <div className="bg-white rounded-[3rem] border border-secondary-ivory shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-secondary-ivory">
          {filteredCommissions.length > 0 ? filteredCommissions.map((c) => (
            <div 
              key={c.skin_id} 
              className="p-8 flex items-center justify-between hover:bg-secondary-ivory/10 transition-all cursor-pointer group"
              onClick={() => setSelectedSale(c)}
            >
              <div className="flex items-center gap-6">
                <div className="w-16 h-16 rounded-[1.5rem] bg-secondary-ivory flex flex-col items-center justify-center text-text-dark group-hover:bg-accent-gold group-hover:text-white transition-all duration-500">
                ...
                <div>
                  <div className="flex items-center gap-3">
                    <p className="text-lg font-black text-text-dark uppercase tracking-tight">Order #{(c.skin_orders?.skin_id || '').slice(0, 8)}</p>
                  ...
                  <p className="text-[10px] text-text-muted font-bold uppercase tracking-widest mt-1 flex items-center gap-2">
                    <User size={12} /> {c.skin_orders?.skin_first_name} {c.skin_orders?.skin_last_name?.[0]}. · {c.skin_orders?.skin_customer_email.split('@')[0].slice(0,3)}***@{c.skin_orders?.skin_customer_email.split('@')[1]}
                  </p>
                </div>
              </div>
              <div className="text-right flex items-center gap-8">
                <div>
                  <p className="text-lg font-black text-text-dark">{formatPrice(c.skin_order_amount)}</p>
